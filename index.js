...
axios.get(INDEX_URL).then(response => {
  guides.push(...response.data.results) // 把results內每一位的資訊一一push入guides陣列中
  renderPaginator(guides.length)
  renderByMode(displayMode, onPage)
  ...
  data.forEach((item) => {
    htmlContent += `
        <div class="col-sm-4 col-md-4 col-lg-3 mt-3 mx-2 d-flex flex-column card">
           ...
           <div class="text-center"><button class="btn btn-info btn-sm btn-add-favorite" data-id="${item.id}">+</button></div>
           </div>
        </div>
      `
  })
    ...
    rawHTML += `
      <tbody>
        <tr style="height:100px">
          ...
          <td class="text-center"><button class="btn btn-info btn-sm btn-add-favorite" data-id="${item.id}">+</button></td>
        </tr>
      </tbody>
     `
  })
...
function getGuidesByPage(page) {
  ...
  const data = filtered.length ? filtered : guides    // 如果搜尋結果有東西，條件判斷為 true ，會回傳 filtered，然後用 data 保存回傳值，否則就還是取總清單 Guide
  // 回傳切割後的新陣列
...
function addToFavorite(id) {
  const list = JSON.parse(localStorage.getItem('favoriteGuides')) || []
  const addGuide = guides.find((guide) => guide.id === id)
  if (list.some((guide) => guide.id === id)) {
    return alert('已加入至清單中')
  }
  list.push(addGuide)
  localStorage.setItem('favoriteGuides', JSON.stringify(list))
}
...
paginator.addEventListener('click', function onPaginatorClicked(event) {
  ...
  // 透過 dataset 取得被點擊的頁數
  onPage = Number(event.target.dataset.page)
  // 依當下的 mode 更新畫面
  renderByMode(displayMode, onPage)
})
...
searchForm.addEventListener('submit', function onSearchFormSubmitted(event) {
  ...
  // 設變數儲存search input內搜尋關鍵字
  const keyword = searchInput.value.trim().toLowerCase()
  onPage = 1 // 讓每一次搜尋都是從第一頁呈現畫面
  // 人名篩選
  filtered = guides.filter((guide) => guide.name.trim().toLowerCase().includes(keyword))
  if (filtered.length === 0) {
    return alert('Cannot find guides with keyword:' + keyword)
  } else {
    // 重置分頁器，現在分頁器的長度是根據 filteredMovies 的長度來決定，因此在搜尋完後，要重新呼叫一次 renderPaginator
...
//   if (event.target.matches('.filtered-by-male') || event.target.matches('.filtered-by-female')) { // 分性別
//     if (filteredName.length !== 0) { // 當有搜尋結果時
//       if (event.target.matches('.filtered-by-male')) { // 將搜尋結果篩選男性
//         filtered = filteredName.filter((guide) => guide.gender === "male")
//       } else if (event.target.matches('.filtered-by-female')) { // 將搜尋結果篩選女性
//         filtered = filteredName.filter((guide) => guide.gender === "female")
//       }
//     } else { // 無搜尋結果時 
//       if (event.target.matches('.filtered-by-male')) { // 將全部資料篩選男性
//         filtered = guides.filter((guide) => guide.gender === "male")
//       } else if (event.target.matches('.filtered-by-female')) {  // 將全部資料篩選女性
...
//     } else {  // 無搜尋結果時 
//       filtered = [] // filtered 為空陣列(因為也無篩選性別)
//       // searchInput.value = ''  // 
//       renderPaginator(guides.length)
//     }
//   }
...
home.addEventListener('click', function onHomeClicked(event) {
  filtered = []
  searchInput.value = ''
  displayMode = "card"
  onPage = 1
